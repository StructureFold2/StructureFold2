        ...
        try:
            ...
            if len(value_windows) > 1:
                for j in range(0, len(value_windows)):
                    tidbit = tuple([get_sum(value_windows[j]),get_sum(abs_value_windows[j]) ,seq_windows[j],k,str(j)])
...
def main():
    ...
    parser.add_argument('-wlen',type=int, default=50, help='<int> [default = 50] Window Length')
